...
# entrada dos telefones (copiados e colado da lista)
telefones = '''
...
# cada telefone é agora um elemento de uma lista
lista_telefones = telefones.split()

# seleção apenas dos telefone válidos, de acordo com as regras impostas
telefones_validos = []
for telefone in lista_telefones:
    # não pode haver dois dígitos consecutivos idênticos
    digitosConsecutivosIdenticos = False
    for i in range(len(telefone)-1):
        if telefone[i] == telefone[i+1]:
            digitosConsecutivosIdenticos = True
            break
    ...
    if soma%2 == 0:
        ...
        somaDigitosPar = False
    # o último dígito não pode ser igual ao primeiro
    if telefone[0] == telefone[-1]:
        ultimoDigitoigualPrimeiro = True
    else:
        ultimoDigitoigualPrimeiro = False

    # teste das condições para adicionar o número válido na lista
    if digitosConsecutivosIdenticos == False and somaDigitosPar == True and ultimoDigitoigualPrimeiro == False:
        telefones_validos.append(telefone)

# exibição dos número válidos
print("Telefones válidos = %d" % len(telefones_validos))
for i in range(len(telefones_validos)):
    print("%02d -- %s" % (i+1, telefones_validos[i]))
